...
int main(int argc, char** argv)
{
	...
	if (input->display)
	{
		...
		else
		{
			...
			// Usa SEMPRE maxBias nel nome del file "true"
			sprintf(truefile, "tr_%d_%d_%g_32.ds2",
					input->numPages, input->maxBias, input->alfaB);

			VECTOR res = load_data(truefile, &righe, &d);
